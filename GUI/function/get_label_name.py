class GetLabelName:
    defect_name_zh = ['灰塵', '毛髮', '橘皮', '溢漆', '紅色記號', '刮傷', '觸摸痕']
    defect_name_en = ['Dirt', 'Hair', 'Orange peel', 'Overspray',
                      'Red mark', 'Sanding scratches', 'Touch mark']

    label_defect_direction_zh = ['正下', '左下', '左上', '正上', '右上', '右下']
    label_defect_direction_en = ['Bottom', 'Left Bottom', 'Left Top',
                                 'Top', 'Right Top', 'Right Bottom']

    label_left = ['label_camera_left1', 'label_camera_left2', 'label_camera_left3',
                  'label_camera_left4', ' label_camera_left5', 'label_camera_left6']
    label_right = ['label_camera_right1', 'label_camera_right2', 'label_camera_right3',
                   'label_camera_right4', ' label_camera_right5', 'label_camera_right6']

    label_defect_check = ['label_check_dirt', 'label_check_hair',
                          'label_check_orange_peel',
                          'label_check_overspray',
                          'label_check_redmark',
                          'label_check_sanding_scratches',
                          'label_check_touch_mark']
    label_defect_location = ['label_location_dirt', 'label_location_hair',
                             'label_location_orange_peel',
                             'label_location_overspray',
                             'label_location_redmark',
                             'label_location_sanding_scratches',
                             'label_location_touch_mark']
    label_defect_rate = ['label_rate_dirt', 'label_rate_hair',
                         'label_rate_orange_peel',
                         'label_rate_overspray',
                         'label_rate_redmark',
                         'label_rate_sanding_scratches',
                         'label_rate_touch_mark']
